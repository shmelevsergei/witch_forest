...
export class TilledCollision extends Component {
    ...
    ClearColliders() {
        this.tiledMap.getLayer("Ground").getComponentsInChildren(BoxCollider2D).forEach(element => {
            element.destroy();
        })
    }
    ...
    CreateCollision() {
        ...
        const x = groundLayer.getLayerSize().width;
        const y = groundLayer.getLayerSize().height;
